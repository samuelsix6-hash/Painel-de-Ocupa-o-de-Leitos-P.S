...
const HistoryTable: React.FC<HistoryTableProps> = ({ data, filteredDates, selectedMonth, onMonthChange, onDelete, highlightedDateKey, isAdmin }) => {
    ...
    return (
        <div>
            ...
            {filteredDates.length === 0 ? (
                 <p className="text-gray-500 py-8 text-center bg-gray-50 rounded-md border border-dashed border-gray-300 font-medium">
                ...
                <div className="overflow-x-auto max-h-[500px] border rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200 relative border-collapse">
                        <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-black text-slate-500 uppercase tracking-wider">
                                    Data
                                </th>
                                {bedTypes.map(type => (
                                    <th key={type} scope="col" className="px-6 py-3 text-center text-xs font-black text-slate-500 uppercase tracking-wider">
                                        {type.replace('Leitos ', '').replace('Clínicos', 'Clín.').replace('Pediátricos', 'Ped.')}
                                    </th>
                                ))}
                                {isAdmin && (
                                    <th scope="col" className="px-6 py-3 text-center text-xs font-black text-slate-500 uppercase tracking-wider">
                                        Ações
                                    </th>
                                )}
                            </tr>
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                            {filteredDates.map(dateStr => {
                                ...
                                const formattedDate = new Date(`${dateStr}T00:00:00`).toLocaleDateString('pt-BR');
                                return (
                                    <tr key={dateStr} className={`hover:bg-blue-50/30 transition-colors ${dateStr === highlightedDateKey ? 'animate-flash' : ''}`}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-700">
                                            {formattedDate}
                                        </td>
                                        {bedTypes.map(type => (
                                            <td key={`${dateStr}-${type}`} className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 text-center font-medium">
